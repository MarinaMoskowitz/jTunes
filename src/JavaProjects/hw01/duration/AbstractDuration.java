package JavaProjects.hw01.duration;
...
abstract class AbstractDuration implements Duration {
  ...
  /**
   * Formats a sequence of uninterpreted characters and two-character format specifiers.
   *
  ...
  public String format(String template) {
    ...
    for (int i = 0; i < template.length(); i++) {
      if (template.charAt(i) == '%') {
        // Special case for the final character
        if (i == template.length() - 1) {
          throw new IllegalArgumentException("error: malformed template ('%' at end of template)");
        }
        ...
        switch (template.charAt(i)) {
          case 't':
            newString += (Long.toString(this.inSeconds()));
            break;
          case 'H':
            if (this.hoursOf(this.inSeconds()) < 10) {
              newString += "0";
            }
            newString += Integer.toString(this.hoursOf(this.inSeconds()));
            break;
          case 'h':
            newString += Integer.toString(this.hoursOf(this.inSeconds()));
            break;
          case 'M':
            ...
            newString += Integer.toString(this.minutesOf(this.inSeconds()));
            break;
          case 'm':
            newString += Integer.toString(this.minutesOf(this.inSeconds()));
            break;
          case 'S':
            ...
            newString += Integer.toString(this.secondsOf(this.inSeconds()));
            break;
          case 's':
            newString += Integer.toString(this.secondsOf(this.inSeconds()));
            break;
          case '%':
            ...
            break;
          default:
            throw new IllegalArgumentException("error: malformed template");
        }
      } else {
        newString += template.charAt(i);
      }
    }
